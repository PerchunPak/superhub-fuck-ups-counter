...
<div class="flex absolute top-8 lg:top-12 xl:top-14 right-14 xl:right-20">
	<div
		class="rounded border-4 border-secondary bg-color-background h-fit p-1 w-40 grid gap-2 grid-cols-3"
	>
	...
	<a
		href="https://github.com/PerchunPak/superhub-fuck-ups-counter"
		class="ml-2 grid items-center rounded"
	>
		<svg width="40" height="40" viewBox="0 0 98 96" xmlns="http://www.w3.org/2000/svg">
			<path
				fill-rule="evenodd"
				clip-rule="evenodd"
...
{#if ready}
	<div class="grid place-items-center pt-20 text-center" transition:slide={{ duration: 1000 }}>
	...
	{:else if $nodes.length === 0}
		<div role="status" class="pt-20" out:blur>
			<svg
				aria-hidden="true"
	...
	{:else}
		<div class="pt-10 max-w-5xl" transition:slide={{ delay: 800, duration: 3000 }}>
					...
					<div class="bg-secondary rounded-md px-4 py-5 w-full relative">
						...
						<p>
							...
							<span class="font-bold"
								>{formatUnixTimestamp(
									$locale,
									calculateTotalDowntime(node.fuckUps),
									'amount of time'
								)}</span
							>.
						</p>
						...
						{:else}
							<p>
								...
								<span class="font-bold"
									>{formatUnixTimestamp(
										$locale,
										node.fuckUps.slice(-1)[0].start,
										'from now with suffix'
									)}</span
								>.
							</p>
						{/if}
